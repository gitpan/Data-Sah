package Data::Sah::Manual::Schema; # just to make PodWeaver happy

1;
# ABSTRACT: Description of Sah schema language
...
used attribute is the B<value> attribute. So common that, when specifying a
clause in a schema, only its C<value> attribute is specified unless when using
the normalized form.
...
Aside from the C<value> attribute, there are others, serving various functions.
...
If you don't have EXTRA_STUFFS, you can also write it like this (saves a couple
of characters):
...
EXTRA_STUFFS is a hashref. Currently the only known key is B<def>, for defining
base schemas locally (see L</"BASE SCHEMA"> for more information). The other
keys are reserved for future use.
...
You can also declare base schemas/types locally using the B<def> key in
EXTRA_STUFFS (the third element of the array schema), for example:
...
(This is Huffman encoding principle at work, since the B<value> attribute is the
most common, it has the shortest syntax.)

Attribute can also be specified separately (they will be combined during
normalization).
...
L<Data::Sah::Compiler::human> compiler to supply description. You can add
translations by adding more attributes.
...
'[merge:+]', '[merge:!]' and so on. In merging, Data::ModeMerge allows keys on
the right side hash not only to replace but also add, subtract, remove keys from
the left side. This is powerful because it allows schema definition to not only
add clauses (restrict types even more), but also replace clauses (change type
